...
const CartTeamChampion8Team = (props) => {
  const classes = useStyles();
  const { handleOpenModal, groupIndex, roundData, name } = props;
  ...
  const DrawA1 =
    ...
    GroupesEquipe.find((el) =>
      el.order.find((data) => +data.equipe_id === +draw["champion"])
    );
  ...
  const equipe1 =
    DrawA1 &&
    DrawA1.order.find((data) => +data.equipe_id === +draw["champion"]);
  ...
  return (
    <Grid item xs={12}>
      {user.role === "employee" &&
        ...
        correctDraw &&
        correctDraw.champion ? (
          <>
            {+draw[roundData[groupIndex]] === +correctDraw.champion ? (
              <Card
      ...
      {user.role === "employee" &&
        (draw && correctDraw && correctDraw.champion ? (
          <>
            {+draw[roundData[groupIndex]] ===
            +correctDraw[roundData[groupIndex]] ? (
              ...
              >
                {drawSetting8TeamEmployee.correctChampion} points
              </span>
            ) : (
              <span
                style={{
...
export default CartTeamChampion8Team;
