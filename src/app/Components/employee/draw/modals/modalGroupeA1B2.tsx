...
const ModalGroupeA1B2 = (props) => {
  ...
  const DrawA1 =
    ...
    GroupeEquipe.find((el) =>
      el.order.find((data) => +data.equipe_id === +draw["A1"])
    );

  const equipe1 =
    DrawA1 && DrawA1.order.find((data) => +data.equipe_id === +draw["A1"]);

  const DrawB2 =
    draw &&
    GroupeEquipe.find((el) =>
      el.order.find((data) => +data.equipe_id === +draw["B2"])
    );

  const equipe2 =
    DrawB2 && DrawB2.order.find((data) => +data.equipe_id === +draw["B2"]);
  ...
  const handleSubmit = () => {
    ...
    setSelectedTeam(0);
    console.log(tempDraw[roundData[groupIndex]]);
    handleCloseModal();
  };
...
export default ModalGroupeA1B2;
